...
try:
    ...
    ForgetStrategy = None
    print("Warning: ForgetStrategy not available. Machine unlearning features disabled.")
...
def apply_forget_strategy(df, strategy_name, forget_ratio=0.2, fold=None, **strategy_params):
    """
    应用遗忘策略，将数据集划分为保留集和遗忘集
    
    Args:
        df: 原始数据DataFrame
        strategy_name: 遗忘策略名称
        forget_ratio: 遗忘比例
        fold: 当前fold编号（用于输出文件命名）
    ...
    if ForgetStrategy is None:
        raise ImportError("ForgetStrategy not available. Please ensure forget_strategy.py is accessible.")
    ...
    print(f"策略结果: 总用户{strategy_info['total_users']}, "
...
def main(dname, fname, dataset_name, configf, min_seq_len=3, maxlen=200, kfold=5, 
         gen_forget_data=False, forget_ratio=0.2, **strategy_params):
    """split main function
    ...
    Args:
        ...
        gen_forget_data (bool, optional): If True, generates retain/forget datasets for all strategies. Defaults to False.
        forget_ratio (float, optional): ratio of data to forget. Defaults to 0.2.
        **strategy_params: additional parameters for forget strategy
    ...
    # 如果指定，则为所有策略生成机器遗忘数据集
    if gen_forget_data:
        print("="*20)
        print(f"开始批量生成遗忘策略数据集，遗忘比例: {forget_ratio}")
        ...
        # 定义所有要处理的策略
        all_strategies = [
            'random', 'low_performance', 'high_performance', 
            'low_engagement', 'unstable_performance'
        ]

        # 使用分割好的、包含fold信息的 train_valid 数据 (splitdf)
        # 这是应用遗忘策略的最佳位置，因为它包含了每个用户的完整序列和fold信息
        base_df = splitdf

        for strategy in all_strategies:
            print(f"\n--- 处理策略: {strategy} ---")
            try:
                # 1. 应用遗忘策略，得到保留和遗忘的用户数据
                # 注意：这里我们直接在原始用户序列上划分，而不是在切片后的序列上
                ...
                # 2. 为保留集和遗忘集分别生成序列
                print("  正在为保留集生成序列...")
                retain_seqs = generate_sequences(retain_df, effective_keys, min_seq_len, maxlen)
                
                print("  正在为遗忘集生成序列...")
                forget_seqs = generate_sequences(forget_df, effective_keys, min_seq_len, maxlen)

                # 3. 构建文件名并保存
                # 文件名中不包含fold，因为fold信息已在文件内部
